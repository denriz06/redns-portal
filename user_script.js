...
const sessionRole = localStorage.getItem('sessionRole') || 'user';
// guard dinonaktifkan

let currentUser = localStorage.getItem('sessionUser') || "Guest";
function getBestScoreFromDB() {
    var db = JSON.parse(localStorage.getItem('usersDB') || '{}');
    var ud = db[currentUser] || {};
    return ud.bestScore != null ? ud.bestScore : null;
}
let bestScore = getBestScoreFromDB();
...
window.onload = () => {
    if (title) title.innerText = "USER DASHBOARD";
    initNeonToggle();
    showTab('home');
};
...
window.showTab = (tabName) => {
    ...
    if (tabName === 'home') {
        area.innerHTML = '<div style="padding:15px;border-left:3px solid #00f7ff;background:rgba(0,247,255,0.05);border-radius:0 10px 10px 0;"><h3 style="color:#00f7ff;margin-bottom:5px;">Halo, ' + currentUser + '!</h3><p style="font-size:13px;color:#fff;">ID: <span style="color:#00f7ff;">' + currentUser + '</span></p><p style="font-size:11px;margin-top:8px;opacity:0.8;">Status: System Administrator</p><p style="font-size:11px;opacity:0.8;">Akses: Full Permissions granted.</p>' + scoreWidget + '</div>';
    }
...
window.checkGuess = () => {
    ...
    if (guess === secretNumber) {
        ...
        if (bestScore == null || attempts < bestScore) {
            bestScore = attempts;
            var db = JSON.parse(localStorage.getItem('usersDB') || '{}');
            if (db[currentUser]) { db[currentUser].bestScore = attempts; localStorage.setItem('usersDB', JSON.stringify(db)); }
        }
